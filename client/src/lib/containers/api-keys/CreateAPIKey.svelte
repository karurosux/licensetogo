...
<Dialog.Root bind:open onOpenChange={handleModalClosed}>
	<Dialog.Trigger>
		<Button variant="outline">
			<Plus />
			{$t((ts) => ts.apiKeyManager.createAPIKey.title)}
		</Button>
	</Dialog.Trigger>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>{$t((ts) => ts.apiKeyManager.createAPIKey.title)}</Dialog.Title>
			<Dialog.Description>
				{$t((ts) => ts.apiKeyManager.createAPIKey.description)}
			</Dialog.Description>
		</Dialog.Header>
		{#if failed}
			<Alert.Root variant="destructive">
				<Alert.Description>
					{$t((ts) => ts.apiKeyManager.createAPIKey.createError)}
				</Alert.Description>
			</Alert.Root>
		{/if}
		<form
