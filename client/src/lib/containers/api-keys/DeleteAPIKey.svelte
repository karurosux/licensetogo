<script lang="ts">
	...
	import type { ApikeyRecord } from '$lib/models/generated/pb-models'
...
<Dialog.Root bind:open>
	...
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>{$t((ts) => ts.apiKeyManager.deleteAPIKey.title)}</Dialog.Title>
			<Dialog.Description>
				{$t((ts) => ts.apiKeyManager.deleteAPIKey.message, { name })}
			</Dialog.Description>
		</Dialog.Header>
		<form
