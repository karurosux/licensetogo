<script lang="ts">
	...
	import CreateAPIKey from '$lib/containers/api-keys/CreateAPIKey.svelte'
	import DeleteAPIKey from '$lib/containers/api-keys/DeleteAPIKey.svelte'
	...
	onMount(() => {
		...
		t.subscribe(($t) => {
			breadcrumbs.set([
				{ href: '/', label: $t((ts) => ts.general.home) },
				{ label: $t((ts) => ts.apiKeyManager.title) },
			])
		})
	})
</script>
...
<svelte:head>
	<title>{$t((ts) => ts.apiKeyManager.title)} | {APP_NAME}</title>
</svelte:head>
...
<Card.Root>
	<Card.Header class="px-7">
		<Card.Title>{$t((ts) => ts.apiKeyManager.title)}</Card.Title>
		<Card.Description>{$t((ts) => ts.apiKeyManager.descriptions)}</Card.Description>
	</Card.Header>
	<Card.Content>
					...
					{#each data?.licenses?.items || [] as l (l.id)}
						<Table.Row>
							...
							<Table.Cell class="flex items-center justify-center gap-2">
								...
								<DeleteAPIKey id={l.id} name={l.name} />
							</Table.Cell>
						</Table.Row>
					{/each}
				</Table.Body>
